(ns app.ui.root
  (:require
   ...
   [app.model.calendar :as calendar]
...
(defn get-days-in-month []
  (let [date      (js/Date.)
        ...
        month     (.getMonth date)
        days      (.getDate (js/Date. year (inc month) 0))
        days-coll (range days)
        days-coll (map #(js/Date. year month (inc %)) days-coll)
        days-coll (partition 7 7 [nil] days-coll)]
    days-coll))

(defsc CalBox [_this {:keys [value cal-item add-button]} {:keys [on-new-event]}]
  {}
  (div {:classes [cal-item]}
       (button {:style {}
                :onClick on-new-event
                :classes [add-button]}
               "+")
       (div {:style {:marginLeft "auto"}} (str (if value (.getDate value) "empty") " "))))

(def ui-cal-box (comp/computed-factory CalBox {:keyfn #(or (:value %) (random-uuid))}))

(defsc Calendar [this {:keys [] :as props}]
  {:query         [{:event/id [:event/id :event/title]}]
   :ident         (fn [] [:component/id :settings])
   :route-segment ["settings"]
   :initial-state {}
   :css           [[:.cal-item {:border-right "1px solid #333" :border-bottom "1px solid #333"
                                :height       "100px"          :width         "100px"
                                :display      "flex"           :padding       "8px"}]
                   #_[:.cal-item:hover {:background "linear-gradient(to right, #ffefba, #ffffff)"}]
                   [:.add-button {:border "1px solid #888" :height "22px" :width "22px" :background "#eee"}]
                   [:.add-button:hover {:border "1px solid #000" :background "#aaa" :cursor "pointer"}]]}

  (tap> [::calendar props])

  (let [{:keys [cal-item add-button]} (css/get-classnames Calendar)
        on-new-event                  (fn []
                                        (comp/transact! this [(calendar/upsert-event {:event/id (random-uuid) :event/title (str "event" (rand-int 100))})]))]
    (div :.ui.container.segment
         (h3 "Settings")
         (div
          {:style {:borderLeft "1px solid #333" :borderTop "1px solid #333" :width "fit-content"}}
          (map
           (fn [row]
             (div
              {:style {:display "flex"} :key (hash row)}
              (map (fn [value]
                     (ui-cal-box
                      {:value      value
                       :cal-item   cal-item
                       :add-button add-button}
                      {:on-new-event on-new-event})) row)))
           (get-days-in-month))))))

(dr/defrouter TopRouter [this props]
  {:router-targets [Main Signup SignupSuccess Calendar]})
...
(defsc Root [_this {:root/keys [top-chrome] :as props}]
  {:query         [{:root/top-chrome (comp/get-query TopChrome)}
                   #_{:event/id [:event/id :event/title]}
                   {:all-events [:event/id :event/title]}]
